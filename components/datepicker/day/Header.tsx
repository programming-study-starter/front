...
export default function CustomDatepickerDayHeader({
  onModeChange,
  pickerDate,
  onPickerDateChange,
  selectedDate,
  ...
  return (
    <div className=' w-full flex flex-row gap-2 items-center place-content-center'>
      <Button size={`sm`} color={`none`} className={`w-1/6`} disabled={!isAvailableMove(-1)} onClick={() => { onClickMove(-1); }}><Image src={`/icons/solid/arrows/angle-left.svg`} alt={`left`} width={8} height={8} /></Button>
      <Button size={`sm`} color={`none`} className={`w-4/6 font-bold`} onClick={() => { onModeChange('month') }}>{`${pickerDate.substring(0, 7)}`}</Button>
      <Button size={`sm`} color={`none`} className={`w-1/6`} disabled={!isAvailableMove(1)} onClick={() => { onClickMove(1); }}><Image src={`/icons/solid/arrows/angle-right.svg`} alt={`right`} width={8} height={8} /></Button>
    </div>
  );
}
